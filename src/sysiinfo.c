...
sysintinfo_t *sysinfo_new(int32_t rec_type,int32_t subtype,int32_t size,int32_t count,int32_t *data){
	if(rec_type!=7 || subtype!=3 || size!=4 || count!=8) return NULL;

	sysintinfo_t *ret=malloc(sizeof(sysintinfo_t));
	//header assignment
...
sysintinfo_t *sysinfo_fnew(FILE *handle){
	...
	sysintinfo_t *ret=sysinfo_new(rec_type,subtype,size,count,data);
	free(data);
	return ret;
}

bool sysinfo_destroy(sysintinfo_t *haystack){
